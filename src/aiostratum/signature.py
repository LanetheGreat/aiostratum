...
from .exceptions import (
    SignatureVerificationFailedException, SigningNotAvailableException,
    UnknownSignatureAlgorithmException, UnknownSignatureIdException,
)
...
def generate_keypair():
    if not ecdsa:
        raise SigningNotAvailableException('ecdsa not installed')
...
def jsonrpc_loads_verify(pubkeys, txt):
    ...
    if signature_algo != 'ecdsa;SECP256k1':
        raise UnknownSignatureAlgorithmException('%s is not supported' % signature_algo)

    try:
        pubkey = pubkeys[signature_id]
    except KeyError:
        raise UnknownSignatureIdException("Public key for '%s' not found" % signature_id)
    ...
    if not verify(pubkey, signature, txt):
        raise SignatureVerificationFailedException("Signature doesn't match to given data")
