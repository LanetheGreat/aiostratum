...
from .exceptions import (
    MethodNotFoundException, ProtocolException,
    RemoteServiceException, ServiceException,
)
...
class Protocol(LineOnlyReceiver):
    ...
    def writeJsonResponse(self, data, message_id, use_signature=False, sign_method='', sign_params=[]):
        if use_signature:
            serialized = signature.jsonrpc_dumps_sign(
                self.factory.signing_key, self.factory.signing_id,
                False, message_id, sign_method, sign_params, data, None,
            )
        else:
    ...
    def writeJsonError(self, code, message, traceback, message_id, use_signature=False, sign_method='', sign_params=[]):
        if use_signature:
            serialized = signature.jsonrpc_dumps_sign(
                self.factory.signing_key, self.factory.signing_id,
                False, message_id, sign_method, sign_params, None, (code, message, traceback),
            )
        else:
        ...
        if msg_method:
            ...
            try:
                ...
                if result is None and msg_id is not None:
                    ...
                    raise MethodNotFoundException("Event handler cannot process method '%s'" % msg_method)
            except Exception:
                failure = Failure()
            ...
            try:
                ...
                raise ProtocolException("Lookup for deferred object for message ID '%s' failed." % msg_id)

            # If there's an error, handle it as errback
            # If both result and error are null, handle it as a success with blank result
            if msg_error is not None:
                meta['defer'].errback(RemoteServiceException(msg_error[0], msg_error[1], msg_error[2]))
            else:
