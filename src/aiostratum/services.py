...
from .exceptions import (
    DefaultServiceAlreadyExistException, MethodNotFoundException,
    MissingServiceIsDefaultException, MissingServiceTypeException, MissingServiceVendorException,
    ServiceNotFoundException, UnauthorizedException,
)
...
class ServiceFactory:
    ...
    def _split_method(cls, method):
        ...
        if '[' in service_type:
            ...
            try:
                ...
                raise
                # raise ServiceNotFoundException("Invalid syntax in service name '%s'" % type_name[0])
    ...
    def call(cls, method, params, connection_ref=None):
        try:
            ...
            raise MethodNotFoundException("Method name parsing failed. You *must* use format <service name>.<method name>, e.g. 'example.ping'")
        ...
        try:
            ...
            raise MethodNotFoundException("Method '%s' not found for service '%s'" % (func_name, service_type))
    ...
    def lookup(cls, service_type, vendor=None):
        ...
        if vendor:
            try:
                ...
                raise ServiceNotFoundException("Class for given service type and vendor isn't registered")

        # Lookup for any vendor, prefer default one
        try:
            vendors = cls.registry[service_type]
        except KeyError:
            raise ServiceNotFoundException("Class for given service type isn't registered")
    ...
    def register_service(cls, _cls, meta):
        ...
        if not service_type:
            raise MissingServiceTypeException("Service class '%s' is missing 'service_type' property." % _cls)

        if not service_vendor:
            raise MissingServiceVendorException("Service class '%s' is missing 'service_vendor' property." % _cls)

        if is_default is None:
            raise MissingServiceIsDefaultException("Service class '%s' is missing 'is_default' property." % _cls)
        ...
        if is_default:
            ...
            try:
                ...
                if current.is_default:
                    raise DefaultServiceAlreadyExistException("Default service already exists for type '%s'" % service_type)
...
def admin(func):
    ...
    def inner(*args, **kwargs):
        if not len(args):
            raise UnauthorizedException('Missing password')

        if settings.ADMIN_RESTRICT_INTERFACE is not None:
            ip = args[0].connection_ref()._get_ip()
            if settings.ADMIN_RESTRICT_INTERFACE != ip:
                raise UnauthorizedException('RPC call not allowed from your IP')

        if not settings.ADMIN_PASSWORD_SHA256:
            raise UnauthorizedException('Admin password not set, RPC call disabled')
