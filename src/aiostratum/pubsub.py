...
from .exceptions import AlreadySubscribedException, PubsubException
...
class Pubsub:
    ...
    def subscribe(cls, connection, subscription):
        ...
        if key in session['subscriptions']:
            raise AlreadySubscribedException('This connection is already subscribed for such event.')
    ...
    def get_subscription(cls, connection, event, key=None):
        ...
        if key is None:
            ...
            try:
                ...
                raise PubsubException('Not subscribed for event %s' % event)
